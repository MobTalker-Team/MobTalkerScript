...
/**
 * This class provides an empty implementation of {@link Mts3Listener},
 * which can be extended to create a listener which only needs to handle a subset
 * of the available methods.
 */
public class Mts3BaseListener implements Mts3Listener {
	/**
	 * {@inheritDoc}
	 *
    ...
    @Override public void visit(@NotNull ParseTree tree) {
        if (tree instanceof ErrorNode) {
            visitErrorNode((ErrorNode) tree);
        } else if (tree instanceof TerminalNode) {
            visitTerminal((TerminalNode) tree);
        } else {
            ((ParserRuleContext) tree).enterRule(this);
        }
    }

    @Override public void visitChildren(@NotNull RuleNode node) {
