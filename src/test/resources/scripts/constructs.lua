...
function f (i)
  if typeof(i) ~= 'number' then return i,'jojo'; end;
...
assert(x[1] == 'aloxixi')
x = {f{}}
assert(x[2] == 'jojo' and typeof(x[1]) == 'table')
...
local function allcases (n)
  ...
  for _, v in inext, allcases(n - 1) do
    res[#res + 1] = v
  end
  for i = 1, n - 1 do
    for _, v1 in inext, allcases(i) do
      for _, v2 in inext, allcases(n - i) do
        for _, op in inext, binops do
            res[#res + 1] = {
              "(" .. v1[1] .. op[1] .. v2[1] .. ")",
              op[2](v1[2], v2[2])
            }
...
local level = 1 --_soft and 3 or 4

for _, v in next, allcases(level) do
